...
// Inspired from https://www.youtube.com/watch?v=MvlhMEE9zuc&ab_channel=QixotlLFC
export default function detectCollision(car, otherCar) {
  // We decrease the width and the coordinates a bit to make the collision more real
  car = { ...car, width: 40, x: car.x -10, y: car.y - 10 };
  otherCar = { ...otherCar, width: 40, x: otherCar.x - 10, y: otherCar.y - 10 };
  //Get rotated coordinates for both rectangles
  let tRR = getRotatedSquareCoordinates(car);
  let oRR = getRotatedSquareCoordinates(otherCar);
  ...
  if (sat(thisRectPolygon, otherRectPolygon)) {
    ...
    if (car.angle === 0 && otherCar.angle === 0) {
      if (
        !(
...
function getRotatedSquareCoordinates(square) {
  ...
  let bottomLeft = workOutNewPoints(
    centerX,
    centerY,
    square.x,
  ...
  let bottomRight = workOutNewPoints(
    centerX,
    centerY,
    square.x + square.width,
    square.y + square.height,
